...
function Curll ($url, $post = 0, $headers = 0, $proxy = 0){
        $ch = curl_init();
        $options = array(
            CURLOPT_URL => $url,
...
function ask($dork){
    for ($i = 1;$i <= 201;$i+= 1) {
		$url = "https://www.ask.com/web?q=".rawurlencode($dork)."&qsrc=998&page=".$i;
		$data = Curl($url);
...
return array_unique(array_map("parse",$allLinks));
}
function bing($dork){
    for ($i = 1;$i <= 201;$i+= 10) {
        $url = "https://www.bing.com/search?q=".rawurlencode($dork)."&first=".$i."&FORM=PERE";
		$data = Curl($url);
...
$i = 0;
 while(true){
		...
		$getInfo = Curll("https://cse.google.com/cse.js?hpg=1&cx=partner-pub-2698861478625135:3033704849", 0, $headers);
...
function yahoo($dork){
    for ($i = 1;$i <= 13;$i+= 10) {
        $url = "https://search.yahoo.com/search?q=".rawurlencode($dork)."&b=".$i;
		$data = Curl($url);
...
function wow($dork){
    for ($i = 0;$i <=1001;$i+=7) {
        $url = "https://www.wow.com/search?q=".rawurlencode($dork)."&b=".$i;
		$data = Curl($url);
...
function aol($dork){
    for ($i = 1;$i <=1001;$i+=10) {
        $url = "https://search.aol.com/aol/search?q=".rawurlencode($dork)."&b=".$i;
		$data = Curl($url);
...
function duck($dork){
    for ($i = 0;$i <=50;$i+=10) {
		...
		$getInfo = Curll("https://duckduckgo.com/?q=".rawurlencode($dork), 0, $headers);
		preg_match('#vqd=(.*?)&#si', $getInfo[1], $vqd);
		$json = Curll("https://links.duckduckgo.com/d.js?q=".rawurlencode($dork)."&s=".$i."&vqd=".$vqd[1], 0, $headers);
        preg_match_all('#"http(.*?)"#si', $json[1], $trackUrl);
		if($trackUrl[1] != NULL){
...
function izito($dork){
    for ($i = 1;$i <=5;$i+=1) {
		$url = "https://www.izito.ws/ws?q=".rawurlencode($dork)."&pg=".$i;
		$data = Curl($url);
		preg_match_all('#"http(.*?)" rel="#si', $data, $links);
        if($links[1] != NULL){
