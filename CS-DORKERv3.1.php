...
function yahoo($dork){
    for ($i = 1;$i <= 1001;$i+= 10) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://search.yahoo.com/search?q=".rawurlencode($dork)."&b=".$i);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
...
function wow($dork){
    for ($i = 0;$i <=1001;$i+=7) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://www.wow.com/search?q=".rawurlencode($dork)."&b=".$i);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
...
function aol($dork){
    for ($i = 1;$i <=1001;$i+=10) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://search.aol.com/aol/search?q=".rawurlencode($dork)."&b=".$i);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
...
function duck($dork){
    for ($i = 0;$i <=50;$i+=10) {
		$headers = array();
        $headers[] = 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.'.rand(0000, 3333).'.169 Safari/537.36';
		$getInfo = Curl("https://duckduckgo.com/?q=".rawurlencode($dork), 0, $headers);
		preg_match('#vqd=(.*?)&#si', $getInfo[1], $vqd);
		$json = Curl("https://links.duckduckgo.com/d.js?q=".rawurlencode($dork)."&s=".$i."&vqd=".$vqd[1], 0, $headers);
...
function izito($dork){
    for ($i = 1;$i <=5;$i+=1) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://www.izito.ws/ws?q=".rawurlencode($dork)."&pg=".$i);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
